...
import { ADD_UPLOADS, ADD_ACCOUNT_APP_TRANSFER, FETCH_ACCOUNT_APP_TRANSFER_BY_ID, UPDATE_ACCOUNT_APP_TRANSFER } from 'actions/appTransfer'
...
export default {
  ...
  methods: {
    ...
    async save () {
      ...
      if (this.updatePage) {
        await this.$store.dispatch(UPDATE_ACCOUNT_APP_TRANSFER, {id: this.appTransfer.id, ...appTransfer})
      } else {
        await this.$store.dispatch(ADD_ACCOUNT_APP_TRANSFER, appTransfer)
      }
      await this.$store.dispatch(LOADING, false)
  ...
  async created () {
    ...
    if (id) {
      await this.$store.dispatch(LOADING, true)
      await this.$store.dispatch(FETCH_ACCOUNT_APP_TRANSFER_BY_ID, id)
      if (this.appTransfer) {
